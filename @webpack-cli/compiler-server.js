...
class App {
  ...
  setProxyMiddleware() {
    ...
    if (proxy && type === "[object object]") {
      ...
      Object.keys(proxy).forEach((context) => {
        ...
        console.log("context====", context); // 代理的路径
        console.log("options====", options); // 代理的路径
      ...
      for (let item of proxy) {
        ...
        if (
          ...
          for (let contextItem of context) {
            // console.log("contextItem====", contextItem); // 代理的路径
            // console.log("item====", item); // 代理的路径
            this.app.use(contextItem, createProxyMiddleware(item));
          }
        } else {
          // console.log("context====", context); // 代理的路径
          // console.log("item====", item); // 代理的路径
          this.app.use(context, createProxyMiddleware(item));
        }
      }
    }
  }
  ...
  middleware() {
    ...
    // //   设置缓存服务器 这个需要客户端和服务端一起配置 https://www.npmjs.com/package/webpack-hot-server-middleware
