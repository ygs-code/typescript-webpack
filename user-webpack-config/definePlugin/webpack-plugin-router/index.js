...
`;
      ...
      // import("src/pages/marketing/pages/DiscountCoupon/index.js")  Loadable${this.firstToUpper(name)}
      code.syncComponent += `
import ${this.firstToUpper(name)} from "src${entry}"`;
    ...
    if(!entry){
      ...
      code.routesComponentConfig += `
                    {  
                     ...
                     entry:"${entry}",
                     Component:lazy(
                           () => import(/* webpackChunkName:"${name}" */ "src${entry}")
                      ),
  ...
  ...${fileName},`;
      code.importRoutesConfigCode += `import ${fileName} from "src/${path}";\n`;
